...
from tensorflow.python.client import timeline # timeline
...
class TFTrainer(Trainer):
  ...
  def __init__(self,
    ...
    self.sess = None
    self.timeline_sample_rate = 0.5 # timeline [0, 1], set 0 to no sample, 1 to always sample.
    self.run_options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE) # timeline
    self.run_metadata = tf.RunMetadata() # timeline
    self.global_step = 0
  ...
  def _train_epoch(self, train_op, loss, iterator, idx):
    ...
    timeline_sample = False if (self.timeline_sample_rate <= 0 or np.random.random() >= self.timeline_sample_rate) else True # timeline
    if iterator is not None:
      self.sess.run(iterator.initializer)
      iter = 0
      while True:
        try:
          ...
          if not timeline_sample:
            outs = self.sess.run([train_op, loss],
                                feed_dict=feed_dict)        
          else:
            outs = self.sess.run([train_op, loss],
                                feed_dict=feed_dict,
                                options=self.run_options, # timeline
                                run_metadata=self.run_metadata) # timeline
          end_time = time.time()
          ...
          if timeline_sample: # timeline
            tl = timeline.Timeline(self.run_metadata.step_stats)
            ctf = tl.generate_chrome_trace_format()
            with open('timeline-ep%s-%s.json' % (idx, iter), 'w') as f:
              f.write(ctf)
          iter += 1
        except tf.errors.OutOfRangeError:
          break
      ...
      if timeline_sample: # timeline
        tl = timeline.Timeline(self.run_metadata.step_stats)
        ctf = tl.generate_chrome_trace_format()
        with open('timeline-ep%s.json' % (idx), 'w') as f:
          f.write(ctf)
